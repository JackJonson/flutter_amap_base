...
- (void)onMethodCall:(FlutterMethodCall *)call :(FlutterResult)result {
    ...
    NSNumber *lockedToScreen = (NSNumber *)paramDic[@"lockedToScreen"];
    ...
    _annotation = [[MarkerAnnotation alloc] init];
    _annotation.coordinate = [markerOptions.position toCLLocationCoordinate2D];
    _annotation.title = markerOptions.title;
    _annotation.subtitle = markerOptions.snippet;
    _annotation.markerOptions = markerOptions;
    if ([lockedToScreen  isEqual: @(YES)]) {
         [_mapView removeAnnotation:_annotation];
        _pinView = [[UIImageView alloc] initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width * 0.5-24, [UIScreen mainScreen].bounds.size.height * 0.5-64, 48, 64)];
        _pinView.image = [UIImage imageWithContentsOfFile:[UnifiedAssets getAssetPath:_annotation.markerOptions.icon]];
        _pinView.hidden = NO;
...
@implementation AddMarkers {
    MAMapView *_mapView;
    NSMutableArray<MarkerAnnotation *>* _lastList;
}
- (NSObject <MapMethodHandler> *)initWith:(MAMapView *)mapView {
    _mapView = mapView;
    return self;
...
- (void)onMethodCall:(FlutterMethodCall *)call :(FlutterResult)result {
    ...
    NSString *zoomToSpanCenter = (NSString *) paramDic[@"zoomToSpanCenter"];
    NSString *optionsListJson = (NSString *) paramDic[@"markerOptionsList"];
    NSNumber *clearLast = (NSNumber *) paramDic[@"clearLast"];
    NSString *openAnimation = (NSString *) paramDic[@"openAnimation"];
    NSNumber *clear = (NSNumber *) paramDic[@"clear"];

    NSLog(@"方法marker#addMarkers ios端参数: optionsListJson -> %@", optionsListJson);
    if ([clear  isEqual: @(YES)]) {
        [_mapView removeAnnotations:_mapView.annotations];
    }
    if ([clearLast  isEqual: @(YES)]) {
        [_lastList enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [_mapView removeAnnotation:_lastList[idx]];
        }];
    }
    ...
    for (NSUInteger i = 0; i < rawOptionsList.count; ++i) {
        ...
        [_lastList addObject:annotation];
    }

    [_mapView addAnnotations:_lastList];
    if (moveToCenter) {
        [_mapView showAnnotations:_lastList animated:YES];
    }
    if(zoomToSpanCenter){

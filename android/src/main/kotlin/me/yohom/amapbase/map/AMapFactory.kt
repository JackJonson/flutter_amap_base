...
const val cameraChangedChannelName = "me.yohom/camera_changed"
const val mapTouchChannelName = "me.yohom/map_touch"
...
class AMapView(context: Context,
    ...
    fun setup() {
        ...
        mapView.map.setOnMarkerClickListener {
            eventSink?.success(UnifiedMarkerOptions(it.options, lockedToScreen = false, openAnimation = false).toFieldJson())
            true
        }


        var cameraChangeEventSink: EventChannel.EventSink? = null
        val cameraChangeEventChannel = EventChannel(registrar.messenger(), "$cameraChangedChannelName")
        cameraChangeEventChannel.setStreamHandler(object : EventChannel.StreamHandler {
            override fun onListen(p0: Any?, sink: EventChannel.EventSink?) {
                cameraChangeEventSink = sink
            }
        ...
        mapView.map.setOnCameraChangeListener(object : AMap.OnCameraChangeListener {
            ...
            override fun onCameraChangeFinish(cameraPosition: CameraPosition) {
                cameraChangeEventSink?.success(UnifiedCameraPosition(cameraPosition).toFieldJson())
            }
        })


        var mapTouchEventSink: EventChannel.EventSink? = null
        val mapTouchEventChannel = EventChannel(registrar.messenger(), "$mapTouchChannelName")
        mapTouchEventChannel.setStreamHandler(object : EventChannel.StreamHandler {
            override fun onListen(p0: Any?, sink: EventChannel.EventSink?) {
                mapTouchEventSink = sink
            }

            override fun onCancel(p0: Any?) {}
        })
        mapView.map.setOnMapTouchListener {
            var action: Int=ACTION_CANCEL
            when (it.action) {
                ACTION_DOWN -> action = ACTION_DOWN
                ACTION_MOVE -> action = ACTION_MOVE
                ACTION_UP -> action = ACTION_UP
                ACTION_CANCEL -> action = ACTION_CANCEL
            }
            mapTouchEventSink?.success(UnifiedTouchEvent(motionEvent = it, action = action).toFieldJson())
        }

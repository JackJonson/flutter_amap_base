...
import me.yohom.amapbase.screenInfo
import java.util.*
...
class UnifiedMarkerOptions(
    ...
    constructor(options: MarkerOptions, lockedToScreen: Boolean, openAnimation: Boolean) : this(
    ...
    fun applyTo(map: AMap, lockedToScreen: Boolean, openAnimation: Boolean) {
        val markerOptions: MarkerOptions
        if (lockedToScreen) {
            markerOptions = toMarkerOptionWithoutPosition()
        } else {
            markerOptions = toMarkerOption()
        }
        val marker: Marker = map.addMarker(markerOptions)
        log("screenInfo:$screenInfo")

        if (lockedToScreen && screenInfo != null) {
            marker.setPositionByPixels(screenInfo!![0] / 2, screenInfo!![1] / 2)
            log("setPositionByPixels")
    ...
    fun toMarkerOptionWithoutPosition(): MarkerOptions = MarkerOptions()
            .alpha(alpha)
            .anchor(anchorU, anchorV)
            .draggable(draggable)
            .infoWindowEnable(infoWindowEnable)
...
class UnifiedCameraPosition(
        ...
        private val target: UnifiedLatlng? = null,
        /// 目标可视区域的缩放级别。 [Android, iOS]
        private val zoom: Double = 0.toDouble(),
        /// 目标可视区域的倾斜度，以角度为单位。 [Android]
        private val tilt: Double = 0.toDouble(),
        /// 可视区域指向的方向，以角度为单位，从正北向逆时针方向计算，从0 度到360 度。 [Android]
        private val bearing: Double = 0.toDouble()

) {
    constructor(position: CameraPosition) : this(
            target = UnifiedLatlng(position.target),
            zoom = position.zoom.toDouble(),
            tilt = position.tilt.toDouble(),
...
class UnifiedTouchEvent(
    ...
    constructor(motionEvent: MotionEvent, action: Int) : this(
            action = action,
            x = motionEvent.x,
            y = motionEvent.y,
